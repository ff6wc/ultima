...
import {
  RawStartingItem,
  StartingItemsMetadata,
  StartingItem,
  StartingItems,
} from "~/types/starting_items";
...
const prefilledItems = itemsMetadata.itemsMetadata.reduce(
  (acc, val) => {
    acc[val.id] = val;
    return acc;
  },
...
export const itemSlice = createSlice({
  ...
  reducers: {
    initItemMetadata(state) {
      ...
      const items = itemsMetadata.itemsMetadata.reduce(
        (acc, val) => {
          acc[val.id] = val;
          return acc;
        },
    ...
    setRawStartingItems(state, action: PayloadAction<string>) {
      ...
      const items = Object.entries(startingItems).map(([flag, value]) => {
...
export const { setItems, setRawStartingItems, initItemMetadata } =
  itemSlice.actions;

export const selectStartingItems = (state: AppState) => state.item.items;
